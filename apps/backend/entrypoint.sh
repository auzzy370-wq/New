...
# Neon fallback (in case Railway doesn't inject DATABASE_URL)
...
until npx prisma migrate deploy --schema=./prisma/schema.prisma 2>&1; do
  i=$((i + 1))
  if [ "$i" -ge "$MAX" ]; then
    echo "  migrations timeout — proceeding anyway"
    break
  fi
  echo "  retrying ($i/$MAX)..."
  sleep 2
...
node dist/prisma/seed.js 2>/dev/null || echo "  Seed skipped"
